...
class Manage_Data{
   ...
   public function jquerycustomers_archive(){
	...
	public function accounting_services($segment3){
		...
		$this->filterAndOptions_accounting_services();
		...
		$this->rowHeight = 34;
		$this->page = $page;
		$tableRows = $this->loadTableRows_accounting_services();
		...
		<div class=\"row\">
			<div class=\"col-sm-12 col-sm-12 col-md-7\">
				<div class=\"row\">
					<div class=\"col-sm-12 col-md-6\">
						<h1 class=\"metatitle\">Accounting service List <i class=\"fa fa-info-circle txt16normal\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Accounting service List\"></i></h1>
					</div>				
					<div class=\"col-sm-12 col-md-6\">
						<div class=\"input-group\">
			...
			<div class=\"col-sm-12 col-sm-12 col-md-5\">            
				<h4 class=\"borderbottom\" id=\"formtitle\">Add New Accounting Service</h4>
				<form action=\"#\" name=\"frmaccounting_services\" id=\"frmaccounting_services\" onsubmit=\"return AJsave_accounting_services();\" enctype=\"multipart/form-data\" method=\"post\" accept-charset=\"utf-8\">
					<div class=\"form-group\">
					...
					<div class=\"form-group\">
						<input type=\"hidden\" name=\"accounting_services_id\" id=\"accounting_services_id\" value=\"0\">
						<input type=\"submit\" id=\"submit\" class=\"btn btn-primary backlink\" value=\" Save \" />
						<input type=\"button\" name=\"reset\" id=\"reset\" onClick=\"resetForm_accounting_services();\" value=\"Cancel\" class=\"hidediv btn btn-warning floatleft mleft10\" />
						<input type=\"button\" name=\"archive\" id=\"archive\" value=\"Archive\" class=\"hidediv btn btn-danger floatright\" />
					</div>
				</form>
	...
	public function AJsave_accounting_services(){
		
		$users_id = $_SESSION["users_id"]??0;
		$returnStr = 'Ok';		
		$savemsg = '';
		$accounting_services_id = $_POST['accounting_services_id']??0;
		...
		$duplSql = "SELECT accounting_services_publish, accounting_services_id FROM accounting_services WHERE name = :name";
		$bindData = array('name'=>$name);
		if($accounting_services_id>0){
			$duplSql .= " AND accounting_services_id != :accounting_services_id";
			$bindData['accounting_services_id'] = $accounting_services_id;
		}
		$duplSql .= " LIMIT 0, 1";
		$duplRows = 0;
		$accounting_servicesObj = $this->db->getData($duplSql, $bindData);
		if($accounting_servicesObj){
			foreach($accounting_servicesObj as $onerow){
				$duplRows = 1;
				$accounting_services_publish = $onerow['accounting_services_publish'];
				if($accounting_services_id==0 && $accounting_services_publish==0){
					$accounting_services_id = $onerow['accounting_services_id'];
					$this->db->update('accounting_services', array('accounting_services_publish'=>1), $accounting_services_id);
					$duplRows = 0;
					$returnStr = 'Update';
				}
			}
		...
		if($duplRows>0 || empty($name)){
			...
			if($accounting_services_id==0){
				...
				$accounting_services_id = $this->db->insert('accounting_services', $conditionarray);
				if($accounting_services_id){						
					$returnStr = 'Add';
				}
				else{
					$returnStr = 'Adding new accounting service';
				}
			}
			else{

				$update = $this->db->update('accounting_services', $conditionarray, $accounting_services_id);
				if($update){
					$activity_feed_title = 'Why choose us was edited';
					$activity_feed_link = "/Manage_Data/accounting_services/view/$accounting_services_id";
					...
					$afData = array('created_on' => date('Y-m-d H:i:s'),
									...
									'uri_table_name' => "accounting_services",
									'uri_table_field_name' =>"accounting_services_publish",
									'field_value' => 1);
	...
	public function aJgetPage_accounting_services($segment3){
		...
		$jsonResponse['tableRows'] = $this->loadTableRows_accounting_services();
		
		return json_encode($jsonResponse);
	}
	
	private function filterAndOptions_accounting_services(){
		...
		$filterSql = "FROM accounting_services WHERE accounting_services_publish = 1";
		$bindData = array();
		if($keyword_search !=''){
		...
		$totalRows = 0;
		$strextra ="SELECT COUNT(accounting_services_id) AS totalrows $filterSql";
  	...
  	private function loadTableRows_accounting_services(){
		...
		if($query){
			foreach($query as $onerow){
				...
				//############
				$serviceImgUrl = '';
                $filePath = "./assets/accounts/srvn_$accounting_services_id".'_';
                ...
                if(!empty($serviceImgUrl)){
                    ...
                    $serviceImg = "<button class=\"btn btn-default\" onClick=\"upload_dialog('Upload Accounting Service Thumb', 'accounting_services', 'srvn_$accounting_services_id"."_');\">Upload<button>";
                }
				//############
				
				$str .= "<tr class=\"cursor\">
							<td data-title=\"ID\" onClick=\"getOneRowInfo('accounting_services', $accounting_services_id);\" align=\"right\">$accounting_services_id</td>
							<td data-title=\"Name\" onClick=\"getOneRowInfo('accounting_services', $accounting_services_id);\" align=\"left\">$name</td>
							<td data-title=\"Price\" align=\"left\">$price</td>
							<td data-title=\"Service Thumb\" align=\"left\" id=\"srvn$accounting_services_id\" class=\"positionrelative\">$serviceImg</td>
						</tr>";
			}
		}
		else{
			$str .= "<tr><td class=\"errormsg\" colspan=\"3\">There is no accounting_services meet this criteria</td></tr>";
		}
