...
function filter_Manage_Data_accounting_services(){
	var limit = j('#limit').val();
	var page = 1;
	j("#page").val(page);
	...
	j.ajax({method: "POST", dataType: "json",
		url: "/Manage_Data/aJgetPage_accounting_services/filter",
		data: jsonData,
	}).done(function(data){
	...
	.fail(function(){
		connection_dialog(filter_Manage_Data_accounting_services);
	});
}

function loadTableRows_Manage_Data_accounting_services(){
	var jsonData = {};
	jsonData['keyword_search'] = j('#keyword_search').val();			
	jsonData['totalRows'] = j('#totalTableRows').val();
	jsonData['rowHeight'] = j('#rowHeight').val();	
	...
	j("body").append('<div class="disScreen"><img src="/assets/images/ajax-loader.gif"></div>');
	j.ajax({method: "POST", dataType: "json",
		url: "/Manage_Data/aJgetPage_accounting_services",
		data: jsonData,
	}).done(function( data ) {
		if(data.login != ''){window.location = '/'+data.login;}
		else{
	...
	.fail(function() {			
		connection_dialog(loadTableRows_Manage_Data_accounting_services);
	});
}

function AJsave_accounting_services(){
	j("#submit").val('Saving...').prop('disabled', true);
	j("body").append('<div class="disScreen"><img src="/assets/images/ajax-loader.gif"></div>');
	j.ajax({method: "POST", dataType: "json",
		url: '/Manage_Data/AJsave_accounting_services/',
		data: j("#frmaccounting_services").serialize(),
	})
	.done(function( data ) {
		if(data.login !=''){window.location = '/'+data.login;}
		else if(data.savemsg !='error' && (data.returnStr=='Add' || data.returnStr=='Update')){
			resetForm_accounting_services();
			if(data.returnStr=='Add'){
			...
			filter_Manage_Data_accounting_services();	
			j("#submit").val('Add').prop('disabled', false);			
		}
		else{
			alert_dialog('Alert message', data.returnStr, 'Ok');			
	...
	.fail(function() {		
		connection_dialog(AJsave_accounting_services);
	});
	return false;
}

function resetForm_accounting_services(){
	document.getElementById('formtitle').innerHTML = 'Add New Why choose us';
	document.getElementById('accounting_services_id').value = 0;
	document.getElementById('name').value = '';
